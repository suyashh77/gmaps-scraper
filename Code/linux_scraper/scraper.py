...
import playwright
from playwright.async_api import Browser, BrowserContext, Page, async_playwright, TimeoutError as PlaywrightTimeoutError, Error as PlaywrightError
...
class GoogleMapsScraper:
    def __init__(
        ...
        # Proxy rotation for IP-based scraping (no auth needed)
        self._proxy_list: list[dict] = config.get("proxy", {}).get("servers", [])
    ...
    async def start_browser(self, auth_file: str | None = None) -> None:
        ...
        if resolved_auth:
            ...
            self.logger.info("No auth session — using IP rotation mode")

        # Build context args - add proxy if configured
        context_args = {
        ...
        # Add proxy if configured for IP rotation
        proxy = self._get_next_proxy()
        if proxy:
            context_args["proxy"] = proxy
            self.logger.info(f"Using proxy: {proxy.get('server', 'unknown')}")
    ...
    def _get_next_proxy(self) -> dict | None:
        """Return next proxy from rotation list, or None if no proxies configured."""
        if not self._proxy_list:
            return None
        proxy = self._proxy_list[self._proxy_index % len(self._proxy_list)]
        self._proxy_index += 1
        self._proxy_rotation_count += 1
        # Build proxy dict for Playwright
        if isinstance(proxy, str):
            return {"server": proxy}
        return proxy

    def rotate_proxy(self) -> None:
        """Force proxy rotation on next browser restart."""
        if self._proxy_list:
            self._proxy_index += 1
            self.logger.info(f"Rotating to next proxy (index {self._proxy_index % len(self._proxy_list)})")
    ...
    async def _extract_review(self, el: Any) -> dict[str, Any] | None:
        try:
            ...
            except (PlaywrightTimeoutError, PlaywrightError, ValueError, IndexError):
                pass
            try:
    ...
    async def scrape_single_store(self, store: Any) -> str:
        ...
        effective_target = max(target_reviews - master_reviews, 0)
        self.logger.info(
